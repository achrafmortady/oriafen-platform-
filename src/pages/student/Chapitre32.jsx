...
const VIDEO_URL  = `${SUPABASE}/oriafen_ch32_video.mp4`
const TOTAL_SLIDES = 9
const SLIDE_URLS = Array.from({ length: TOTAL_SLIDES }, (_, i) =>
  `${SUPABASE}/ch32_slide_${String(i + 1).padStart(2, '0')}.jpg`
)
    ...
    return (
      <div style={{ textAlign: 'center', padding: '32px 24px' }}>
        ...
        <p style={{ fontSize: 14, color: '#6b7280', margin: '8px 0 24px', fontFamily: 'Montserrat, sans-serif' }}>
          {result.passed ? 'Vous avez validé le Chapitre 3.2. Excellent travail !' : `Score minimum requis : ${MIN_PASS}/${total}. Révisez les slides et retentez.`}
        </p>
...
// ─── Main ─────────────────────────────────────────────────────
export default function Chapitre32({ onComplete, isCompleted }) {
    ...
    return (
      <div style={{ fontFamily: 'Montserrat, sans-serif', display: 'flex', flexDirection: 'column', gap: 16 }}>
        <div style={{ background: '#fff', borderRadius: 16, border: '2px solid #c9a84c', padding: '28px 32px', textAlign: 'center', boxShadow: '0 4px 20px rgba(201,168,76,0.12)' }}>
          ...
          <h3 style={{ fontWeight: 800, fontSize: 20, color: '#1a3d2b', marginBottom: 6, fontFamily: 'Cormorant Garamond, serif' }}>Chapitre 3.2 complété !</h3>
  ...
  return (
    <div style={{ fontFamily: 'Montserrat, sans-serif', display: 'flex', flexDirection: 'column', gap: 0 }}>
      ...
      <div style={{ background: 'linear-gradient(135deg, #1a3d2b 0%, #0d2818 100%)', borderRadius: 16, padding: '22px 28px', marginBottom: 20, border: '1px solid rgba(201,168,76,0.25)', position: 'relative', overflow: 'hidden' }}>
        ...
        <p style={{ fontSize: 11, fontWeight: 700, color: '#c9a84c', textTransform: 'uppercase', letterSpacing: '0.12em', margin: '0 0 4px' }}>Unité 3 · Chapitre 3.2</p>
        <h2 style={{ fontSize: 20, fontWeight: 700, color: '#fff', margin: '0 0 4px', fontFamily: 'Cormorant Garamond, serif' }}>Bases du patrimoine — Retraite et ingénierie patrimoniale</h2>

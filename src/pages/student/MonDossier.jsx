...
function DocRow({ required, doc, uploading, onUpload }) {
  ...
  const rowBg = { valid:'rgba(16,185,129,0.06)', pending:'rgba(245,158,11,0.06)', missing:'rgba(239,68,68,0.06)', correction:'rgba(249,115,22,0.06)', none:'rgba(255,255,255,0.03)' }[status] ?? 'rgba(255,255,255,0.03)'
  const rowBorder = { valid:'rgba(16,185,129,0.2)', pending:'rgba(245,158,11,0.2)', missing:'rgba(239,68,68,0.2)', correction:'rgba(249,115,22,0.2)', none:'rgba(255,255,255,0.07)' }[status] ?? 'rgba(255,255,255,0.07)'
  const handleFileChange = (e) => { const file = e.target.files?.[0]; if (file) { onUpload(required.id, required.label, file); e.target.value = '' } }
  return (
    <div style={{ background:rowBg, border:`1px solid ${rowBorder}`, borderRadius:'14px', padding:'14px 16px', transition:'all 0.2s' }}>
      <div style={{ display:'flex', flexWrap:'wrap', alignItems:'center', gap:'12px' }}>
        <div style={{ width:'36px', height:'36px', borderRadius:'10px', flexShrink:0, display:'flex', alignItems:'center', justifyContent:'center', background: status==='valid' ? '#d1fae5' : status==='pending' ? '#fef3c7' : status==='missing' ? '#fee2e2' : '#f3f4f6' }}>
          {status==='valid' ? <CheckCircleIcon className="w-4 h-4" style={{color:'#10b981'}} /> : status==='pending' ? <ClockIcon className="w-4 h-4" style={{color:'#f59e0b'}} /> : status==='missing' ? <XCircleIcon className="w-4 h-4" style={{color:'#ef4444'}} /> : status==='correction' ? <span style={{fontSize:'14px'}}>💬</span> : <span style={{fontSize:'13px', color:'#9ca3af', fontWeight:'700'}}>?</span>}
        </div>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:'8px', flexShrink:0 }}>
          ...
          {status !== 'valid' && (
            <>
              ...
              <button onClick={() => inputRef.current?.click()} disabled={uploading} style={{ display:'flex', alignItems:'center', gap:'6px', padding:'7px 14px', borderRadius:'10px', fontSize:'12px', fontWeight:'600', border:'none', cursor: uploading ? 'not-allowed' : 'pointer', background: uploading ? 'rgba(255,255,255,0.05)' : 'linear-gradient(135deg, #1a4a2e, #2d6b45)', color: uploading ? 'rgba(255,255,255,0.3)' : '#fff', fontFamily:"'Montserrat', sans-serif", boxShadow: uploading ? 'none' : '0 2px 10px rgba(26,74,46,0.4)', transition:'all 0.2s' }}>
                {uploading ? <><svg style={{animation:'spin 0.8s linear infinite',width:'12px',height:'12px'}} viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" opacity="0.25"/><path fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" opacity="0.75"/></svg>Envoi…</> : <><svg style={{width:'12px',height:'12px'}} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>{btnLabel}</>}
              </button>
            </>
          )}
        </div>
      ...
      {(status === 'missing' || status === 'correction') && doc?.rejectionReason && (
        <div style={{ marginTop:'10px', padding:'8px 12px', borderRadius:'10px', fontSize:'11px', fontFamily:"'Montserrat', sans-serif", background: status==='missing' ? 'rgba(239,68,68,0.08)' : 'rgba(249,115,22,0.08)', color: status==='missing' ? '#ef4444' : '#f97316', border: `1px solid ${status==='missing' ? 'rgba(239,68,68,0.2)' : 'rgba(249,115,22,0.2)'}` }}>
          {status==='missing' ? '✕ Raison du rejet : ' : '💬 Correction : '}<strong>{doc.rejectionReason}</strong> — Veuillez renvoyer le document corrigé.
        </div>
      )}
    </div>
  )

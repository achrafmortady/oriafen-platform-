...
import { DownloadIcon, FileIcon, CheckCircleIcon, ClockIcon, XCircleIcon } from '../../components/Icons'

const DOCS_RECUS = [
  { id: 1, name: 'Contrat de mission signé.pdf',   size: '1.4 MB', date: '14/03/2026', type: 'contrat',    icon: '📋' },
  { id: 2, name: 'Convention Oriafen Academy.pdf', size: '0.9 MB', date: '16/03/2026', type: 'convention', icon: '🤝' },
  { id: 3, name: 'Guide démarrage rapide.pdf',     size: '2.1 MB', date: '18/03/2026', type: 'guide',      icon: '📖' },
]

const DOCS_FINAUX = [
  { id: 1, name: 'Numéro ORIAS officiel.pdf' },
  { id: 2, name: 'Attestation immatriculation.pdf' },
  { id: 3, name: 'Certificat IAS1.pdf' },
]

const STATUS_CFG = {
  valid:      { label:'Validé',      bg:'#d1fae5', color:'#10b981', border:'rgba(16,185,129,0.3)', icon: <CheckCircleIcon className="w-4 h-4" style={{color:'#10b981'}} /> },
  pending:    { label:'En attente',  bg:'#fef3c7', color:'#d97706', border:'rgba(245,158,11,0.3)',  icon: <ClockIcon className="w-4 h-4" style={{color:'#d97706'}} /> },
  missing:    { label:'Rejeté',      bg:'#fee2e2', color:'#ef4444', border:'rgba(239,68,68,0.3)',   icon: <XCircleIcon className="w-4 h-4" style={{color:'#ef4444'}} /> },
  correction: { label:'Correction',  bg:'#ffedd5', color:'#f97316', border:'rgba(249,115,22,0.3)', icon: '💬' },
  none:       { label:'Manquant',    bg:'#f3f4f6', color:'#9ca3af', border:'rgba(156,163,175,0.3)', icon: <span style={{fontSize:'13px', color:'#9ca3af', fontWeight:'700'}}>?</span> },
}
...
function DocRow({ required, doc, uploading, onUpload }) {
  ...
  const rowBg = { valid:'#f0fdf4', pending:'#fffbeb', missing:'#fff1f2', correction:'#fff7ed', none:'#ffffff' }[status] ?? '#ffffff'
  const rowBorder = { valid:'rgba(16,185,129,0.2)', pending:'rgba(245,158,11,0.2)', missing:'rgba(239,68,68,0.2)', correction:'rgba(249,115,22,0.2)', none:'#e8e2d6' }[status] ?? '#e8e2d6'
    ...
    <div style={{ background:rowBg, border:`1px solid ${rowBorder}`, borderRadius:'14px', padding:'14px 16px', transition:'all 0.2s' }}>
      <div style={{ display:'flex', flexWrap:'wrap', alignItems:'center', gap:'12px' }}>
        ...
        <div style={{ flex:1, minWidth:0 }}>
          ...
          <p style={{ margin:'2px 0 0', fontSize:'11px', color:'#9ca3af', fontFamily:"'Montserrat', sans-serif" }}>{required.sublabel}</p>
          {doc?.fileName && <p style={{ margin:'3px 0 0', fontSize:'11px', color:'#c49a2a', fontFamily:"'Montserrat', sans-serif" }}>📎 {doc.fileName}</p>}
        </div>
        <div style={{ display:'flex', alignItems:'center', gap:'8px', flexShrink:0 }}>
          ...
          {status !== 'valid' && (
            <>
              <input ref={inputRef} type="file" accept={required.accept} style={{display:'none'}} onChange={handleChange} />
              <button onClick={() => inputRef.current?.click()} disabled={uploading}
                style={{ display:'flex', alignItems:'center', gap:'6px', padding:'7px 14px', borderRadius:'10px', fontSize:'12px', fontWeight:'600', border:'none',
                  ...
                  boxShadow: uploading ? 'none' : '0 2px 10px rgba(26,74,46,0.3)' }}>
                {uploading ? (
                  ...
                  </svg>{status === 'none' || status === 'missing' || status === 'correction' ? 'Envoyer' : 'Remplacer'}</>
                )}
              </button>
            </>
          )}
      ...
      {(status === 'missing' || status === 'correction') && doc?.rejectionReason && (
        <div style={{ marginTop:'10px', padding:'8px 12px', borderRadius:'10px', fontSize:'11px',
          background: status==='missing' ? 'rgba(239,68,68,0.06)' : 'rgba(249,115,22,0.06)',
          color: status==='missing' ? '#ef4444' : '#f97316',
          border: `1px solid ${status==='missing' ? 'rgba(239,68,68,0.2)' : 'rgba(249,115,22,0.2)'}`,
          fontFamily:"'Montserrat', sans-serif" }}>
          {status==='missing' ? '✕ Rejeté : ' : '💬 Correction : '}<strong>{doc.rejectionReason}</strong> — Veuillez renvoyer.
        </div>
      )}
    </div>
  )
...
function Toast({ toasts, onDismiss }) {
  return (
    <div style={{ position:'fixed', top:'1rem', right:'1rem', zIndex:50, display:'flex', flexDirection:'column', gap:'8px', pointerEvents:'none', maxWidth:'360px' }}>
      {toasts.map(t => (
        <div key={t.id} style={{ display:'flex', alignItems:'flex-start', gap:'10px', padding:'12px 16px', borderRadius:'14px',
          ...
          backdropFilter:'blur(12px)', boxShadow:'0 8px 30px rgba(0,0,0,0.15)', pointerEvents:'auto',
          ...
          <button onClick={()=>onDismiss(t.id)} style={{background:'none',border:'none',cursor:'pointer',opacity:0.5,color:'inherit',fontSize:'14px'}}>✕</button>
        </div>
      ))}
    </div>
  )
...
export default function MesDocuments() {
  ...
  const [activeTab,   setActiveTab]   = useState('orias')
  ...
  const handleDocChange = useCallback((updated) => {
    ...
    setDocs(prev => {
      ...
      if (old?.status && old.status !== newStatus) {
        ...
        else if (newStatus === 'missing') pushToast(`${label} rejeté.${rejectionReason ? ` ${rejectionReason}` : ''}`, 'error', 8000)
  ...
  useEffect(() => {
    ...
    fetchDocumentsByCategory(user.id).then(setDocs).finally(() => setLoadingData(false))
  ...
  const handleUpload = async (categoryId, categoryLabel, file) => {
    ...
    else if (result.success) { setDocs(prev => ({ ...prev, [categoryId]: result.doc })); pushToast(`${categoryLabel} envoyé ✓`, 'success') }
    else pushToast(`Erreur : ${result.error ?? 'réessayez.'}`, 'error')
  ...
  const TABS = [
    { id: 'orias',  label: '📤 Documents ORIAS',  count: REQUIRED_DOCUMENTS.length },
  ...
  return (
    <>
      <style>{`
        ...
        .doc-dl-btn:hover { color: #1a3d2b !important; background: #f0f9f4 !important; }
      `}</style>
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:'24px', fontFamily:"'Montserrat', sans-serif" }}>
        ...
        <div style={{ background:'linear-gradient(135deg, #1a3d2b 0%, #0d2818 100%)', border:'1px solid rgba(201,168,76,0.2)', borderRadius:'20px', padding:'28px', boxShadow:'0 4px 30px rgba(0,0,0,0.15)' }}>
          ...
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:'16px' }}>
            <div>
              <h2 style={{ margin:0, fontSize:'20px', fontWeight:'700', color:'#ffffff' }}>Centre de Documents</h2>
            ...
            <div style={{ display:'flex', gap:'10px', flexWrap:'wrap' }}>
              {[
                ...
                { label:'Manquants',  value: missingDocs, color:'rgba(255,255,255,0.5)' },
                ...
                <div key={label} style={{ background:'rgba(255,255,255,0.06)', border:'1px solid rgba(255,255,255,0.08)', borderRadius:'12px', padding:'8px 16px', textAlign:'center' }}>
                  <p style={{ margin:0, fontSize:'20px', fontWeight:'700', color }}>{value}</p>
                  <p style={{ margin:0, fontSize:'10px', color:'rgba(255,255,255,0.4)' }}>{label}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          {TABS.map(t => (
            <button key={t.id} className="tab-btn" onClick={() => setActiveTab(t.id)}
              style={{ padding:'10px 20px', borderRadius:'12px', fontSize:'13px', fontWeight:'600',
                border: activeTab===t.id ? '1px solid rgba(201,168,76,0.4)' : '1px solid #e8e2d6',
                background: activeTab===t.id ? 'linear-gradient(135deg, #1a3d2b, #2d6b45)' : '#ffffff',
                color: activeTab===t.id ? '#ffffff' : '#4b5563',
                cursor:'pointer', transition:'all 0.2s',
                boxShadow: activeTab===t.id ? '0 4px 15px rgba(26,61,43,0.25)' : 'none',
                display:'flex', alignItems:'center', gap:'8px' }}>
              {t.label}
              <span style={{ padding:'2px 8px', borderRadius:'20px', fontSize:'11px', fontWeight:'700',
                background: activeTab===t.id ? 'rgba(255,255,255,0.2)' : '#f3f4f6',
                color: activeTab===t.id ? '#fff' : '#6b7280' }}>{t.count}</span>
            </button>
          ))}
        </div>
        ...
        {activeTab === 'orias' && (
          <div style={{ background:'#ffffff', border:'1px solid #e8e2d6', borderRadius:'20px', padding:'24px', boxShadow:'0 2px 12px rgba(0,0,0,0.06)', animation:'fadeIn 0.3s ease' }}>
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'8px' }}>
              <p style={{ margin:0, fontSize:'11px', fontWeight:'600', letterSpacing:'1.5px', textTransform:'uppercase', color:'#c49a2a' }}>Documents requis pour votre dossier ORIAS</p>
            </div>

            {/* Progress */}
            <div style={{ marginBottom:'20px' }}>
              <div style={{ display:'flex', justifyContent:'space-between', marginBottom:'6px' }}>
                <span style={{ fontSize:'12px', color:'#6b7280' }}>Complétude</span>
                <span style={{ fontSize:'12px', fontWeight:'700', color:'#1a3d2b' }}>{validDocs} / {REQUIRED_DOCUMENTS.length} — {progressPct}%</span>
              </div>
            ...
            <div style={{ marginTop:'16px', display:'flex', alignItems:'flex-start', gap:'10px', padding:'14px 16px', borderRadius:'14px', background:'#fefce8', border:'1px solid #fde68a', fontSize:'12px', color:'#78716c' }}>
              <span>ℹ️</span>
              <p style={{margin:0, lineHeight:'1.6'}}>Formats acceptés : <strong style={{color:'#b45309'}}>PDF, JPG, PNG</strong> — max 10 Mo. Vérifiés sous <strong style={{color:'#b45309'}}>24–48h</strong>.</p>
            </div>
          </div>
        )}
        ...
        {activeTab === 'recus' && (
          <div style={{ background:'#ffffff', border:'1px solid #e8e2d6', borderRadius:'20px', padding:'24px', boxShadow:'0 2px 12px rgba(0,0,0,0.06)', animation:'fadeIn 0.3s ease' }}>
            <p style={{ margin:'0 0 6px', fontSize:'11px', fontWeight:'600', letterSpacing:'1.5px', textTransform:'uppercase', color:'#c49a2a' }}>Documents envoyés par Oriafen</p>
            <p style={{ margin:'0 0 20px', fontSize:'12px', color:'#9ca3af' }}>Contrats, conventions et guides de votre conseiller</p>
            <div style={{ display:'flex', flexDirection:'column', gap:'10px' }}>
              {DOCS_RECUS.map(doc => (
                <div key={doc.id}
                  ...
                  <button className="doc-dl-btn"
                    style={{ display:'flex', alignItems:'center', gap:'6px', padding:'8px 14px', borderRadius:'10px', background:'none', border:'1px solid #e8e2d6', cursor:'pointer', color:'#6b7280', fontSize:'12px', fontWeight:'600', transition:'all 0.2s' }}>
        ...
        {activeTab === 'finaux' && (
          <div style={{ background:'#ffffff', border:'1px solid #e8e2d6', borderRadius:'20px', padding:'24px', boxShadow:'0 2px 12px rgba(0,0,0,0.06)', animation:'fadeIn 0.3s ease' }}>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:'10px' }}>
              {DOCS_FINAUX.map(doc => (
                <div key={doc.id}
                  ...
                  <div style={{ width:'44px', height:'44px', borderRadius:'12px', background:'#f3f4f6', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, fontSize:'20px' }}>🔒</div>
                  <div style={{ flex:1 }}>
                  ...
                  <span style={{ padding:'4px 10px', borderRadius:'20px', fontSize:'11px', fontWeight:'600', background:'#f3f4f6', color:'#9ca3af', border:'1px solid #e5e7eb' }}>Verrouillé</span>
                </div>
              ))}
            </div>
            <div style={{ marginTop:'20px', padding:'20px', borderRadius:'16px', background:'linear-gradient(135deg, rgba(26,61,43,0.04), rgba(201,168,76,0.04))', border:'1px solid rgba(201,168,76,0.15)', textAlign:'center' }}>
              ...
              <p style={{ margin:0, fontSize:'12px', color:'#6b7280', lineHeight:'1.6' }}>Une fois votre immatriculation obtenue, tous ces documents seront disponibles.</p>
            </div>
          </div>
        )}
      </div>

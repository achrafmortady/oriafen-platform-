// Restauration V1 -> V2 (audit 2026-09-19) : src/pages/student/FormationCommerciale.jsx
// (784 lignes, fichier live non modifié) était accessible via l'onglet
// "Vente & Scripts" côté client, mais V2 (ClientSpace, LocalCRM.jsx)
// affichait un placeholder générique pour cet onglet. Reproduit ici
// TEL QUEL (scripts d'appel, scripts produits, objections, simulation
// d'appel, évaluation + certificat) — seules différences : `useAuth()`
// remplacé par des props (clientId/clientName, aucun AuthProvider dans le
// shell V2) et `saveExamResult` (src/lib/api.js, live) remplacé par
// formationProgressStore.js (localStorage, même store que Formation IAS1,
// examType='commercial' pour rester distinct). CALL_SCRIPTS/PRODUCT_SCRIPTS/
// OBJECTIONS/CALL_SCENARIOS/getRandomQuiz réutilisés tels quels depuis
// src/data/mockData.js (contenu réel V1).
import { useState } from 'react'
import { saveExamResult } from './formationProgressStore'
import {
  CALL_SCRIPTS,
  PRODUCT_SCRIPTS,
  OBJECTIONS,
...
// ─────────────────────────────────────────────────────────────
function CommercialQuiz({ userName, clientId, onDone }) {
  ...
  const next = async () => {
    ...
    if (current < total - 1) {
      ...
      saveExamResult(clientId, 'commercial', score, total)
...
// MAIN COMPONENT
// ═════════════════════════════════════════════════════════════
export default function LocalFormationCommerciale({ clientId, clientName }) {
  ...
  if (takingQuiz) {
    return (
      <div style={{ fontFamily: 'Montserrat, sans-serif' }}>
        ...
        <CommercialQuiz userName={clientName ?? 'Étudiant'} clientId={clientId} onDone={() => setTakingQuiz(false)} />
      </div>
    )
  }
